...
func TestBuildPagesEmbargo(t *testing.T) {
	...
	t.Run("production excludes future + draft", func(t *testing.T) {
		pages, _, next, err := buildPages(docs, false, now, "/", "", nil, nil, nil, false, nil)
		if err != nil {
			t.Fatal(err)
		}
		got := titles(pages)
	...
	t.Run("preview includes embargoed, marked", func(t *testing.T) {
		pages, _, _, err := buildPages(docs, true, now, "/", "", nil, nil, nil, false, nil)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("after timestamp auto-includes", func(t *testing.T) {
		...
		pages, _, next, err := buildPages(docs, false, later, "/", "", nil, nil, nil, false, nil)
		if err != nil {
			t.Fatal(err)
		}
